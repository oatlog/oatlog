...
program. This is different from egglog, in that Oatlog is more focused on being embeddable and is a
ahead-of-time compiler rather than an interpreter. We find that this has two large consequences.
...
which one can find in per-function or per-basic-block optimization within a compiler. However, this
difference is reduced for large e-graphs, where the interpreter's overhead is amortized over the
data plane.

The second consequence of an ahead-of-time architecture lies in the improved prototypability and
debuggability driven by reading and modifying the Rust code generated by the e-graph compiler. This
aspect simplifies performance engineering for Oatlog, as one can easily separate the problem of
determining what to do from how to do it, in constrast to an interpreter where compiler complexity
mixes with ABI concerns. We could summarize our opinion of the AOT-compiler architectural choice as
a way of simplifying development and performance engineering at the cost of significantly limiting
dynamic rewrite rules and REPL use.
...
e-graph is small enough to fit in some level of cache, but trie indexes have the advantage of
deferring the cache spill through their smaller size. This is possibly the most interesting
implementation detail, from a performance perspective, to study further when comparing Oatlog to
egglog.
...
implementation improvements and larger potential changes to e-graph engine design.
...
- Containers, which are primitive-like accelerators for representing structures such as lists, sets
  ...
  becoming equal. The egglog language as introduced by @egglog does not support containers but
  egglog itself does.
...
- Tiered scheduling of rewrite rules. In a sense actions can be tiered into `union` -- which
  monotonically shrinks the e-graph, `insert` -- which is bounded to $O("eclasses"^"operation arity")$ and in
...
Eqlog is similar, but the language is very desugared -- almost already query planned.
...
Both of the `math` and `boolean_adder` benchmarks grow to very large sizes and are benchmarked by
