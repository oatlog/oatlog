...
pub fn compile_str(s: &str) -> String {
    force_backtrace(|| compile_parse_ok(s.parse::<proc_macro2::TokenStream>().unwrap()).to_string())
}

#[must_use]
pub fn compile_parse_ok(x: proc_macro2::TokenStream) -> proc_macro2::TokenStream {
    force_backtrace(|| compile_egraph_inner(x).unwrap_or_else(|err| err.to_compile_error()))
}
